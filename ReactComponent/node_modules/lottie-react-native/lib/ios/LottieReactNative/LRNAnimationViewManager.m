 ...
 */
RCT_EXPORT_METHOD(playWithCompleteCallcack:(nonnull NSNumber *)reactTag callback:(RCTResponseSenderBlock)callback)
{
    [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        id view = viewRegistry[reactTag];
        if (![view isKindOfClass:[LRNContainerView class]]) {
            ...
            LRNContainerView *lottieView = (LRNContainerView *)view;
            [lottieView playWithCompleteCallcack:^(BOOL isFinished) {
                callback(@[@{@"isFinished": @(isFinished)}]);
            }];
        }
    }];
}
